...
`,
   ...
   * shadcn registry that the framework still depends on. Install these
   * when `pnpm dlx shadcn@latest add <name>` returns 'not found'.
   * ────────────────────────────────────────────── */
  {
    $schema: "https://ui.shadcn.com/schema/registry.json",
    name: "form",
    type: "registry:ui",
    title: "Form (shadcn fallback)",
    description:
      "Canonical shadcn Form primitives (FormField, FormItem, FormLabel, FormControl, FormDescription, FormMessage) for React Hook Form. Installed here because `shadcn add form` no longer ships them upstream.",
