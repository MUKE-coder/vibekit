...
```bash
npx vibekit init --agent all    # write every agent's rules file
npx vibekit init --global       # Claude skill in ~/.claude, not the project
npx vibekit init --force        # overwrite existing files
npx vibekit init --yes          # no prompts (CI)
```
...
├── master_prompt.md             ← Coding standards for your project (installed by `npx vibekit init`)
...
├── cli/                         ← `npx vibekit init` — installs the framework into a project
